...
module.exports = function (app) {
    ...
    app.post('/groups/:groupid(\\d+)/messagetopic/', [auth.CheckAuthToken, auth.CheckInGroup], function (req, res) {
        ...
        if(req.params.groupid && req.body.title && req.body.decoded.UserID && req.body.msg)
        {
            ...
            var insertRequest = "Call addTopic(" + config.pool.escape(req.params.groupid) + ", " + config.pool.escape(req.body.title) + ", " + config.pool.escape(req.body.decoded.UserID) + ", " + config.pool.escape(req.body.msg) + ")";
